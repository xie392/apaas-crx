...
function updateRedirectRules() {
  ...
  // 创建重定向规则
  const rules: chrome.declarativeNetRequest.Rule[] = Object.entries(scriptMappings).map(([from, to], index) => {
...
async function processUploadedFiles(
  ...
  otherFiles: string[],
  zipData: number[],
  progressCallback: (progress: {step: string, percent: number, details?: string}) => void
) {
  try {
    // 开始处理
    ...
    console.log(`Storage: ${storageInfo.usedBytes / 1024 / 1024}MB used of ${storageInfo.quotaBytes / 1024 / 1024}MB quota`);
    ...
    if (totalSizeNeeded > availableSpace) {
      throw new Error(`存储空间不足，需要 ${(totalSizeNeeded / 1024 / 1024).toFixed(2)}MB，但只有 ${(availableSpace / 1024 / 1024).toFixed(2)}MB 可用`);
    }
    ...
    progressCallback({step: "保存JS/CSS", percent: 20, details: "保存主要文件..."});
    ...
    // 从ZIP中选择性地提取JS文件
    if (appState.zipData) {
      progressCallback({step: "提取JS", percent: 80, details: "从ZIP中提取必要的JS文件..."});
      ...
      let remainingSpace = updatedStorageInfo.quotaBytes - updatedStorageInfo.usedBytes;
      const safetyBuffer = 512 * 1024; // 保留 512KB 的安全缓冲区
      remainingSpace -= safetyBuffer;
      
      // 优先保存小文件和可能重要的文件
      const importantPatterns = [
        'chunk-', 'vendor', 'polyfill', 'runtime', 'main', 'app', 'index', 'bundle'
      ];
      
      // 按优先级重新排序
      jsFilesWithSize.sort((a, b) => {
        const aIsImportant = importantPatterns.some(pattern => a.filename.includes(pattern));
        const bIsImportant = importantPatterns.some(pattern => b.filename.includes(pattern));
        ...
        if (estimatedSize < remainingSpace) {
          ...
          if (file) {
            try {
              ...
              progressCallback({
                step: "保存JS", 
                percent: 80 + Math.floor((savedCount / jsFilesWithSize.length) * 10),
    ...
    // 更新重定向规则
    progressCallback({step: "更新规则", percent: 90, details: "更新重定向规则..."});
    updateRedirectRules();
    ...
    progressCallback({step: "错误", percent: 0, details: `处理出错: ${error.message}`});
...
// 辅助函数：保存文件到assets目录
async function saveFileToAssets(fileName: string, fileContent: ArrayBuffer): Promise<void> {
    ...
    chrome.storage.local.get([fileKey], (result) => {
      if (result[fileKey]) {
          ...
          chrome.storage.local.get(chunkKeys, (chunksResult) => {
            try {
              ...
              const blob = new Blob([decompressedData], { 
                type: fileName.endsWith('.js') ? 'application/javascript' : 
              ...
              reader.onerror = () => {
                sendResponse({ exists: false, error: 'Failed to read decompressed data' });
              };
        ...
        if (appState.outputName) {
          ...
          // 检查是否是我们要替换的JS或CSS文件
          if (fileName.endsWith(`${baseOutputName}.umd.js`) || 
              fileName.endsWith(`${baseOutputName}.css`)) {
            
            // 检查对应的文件是否存在
            let matchedFileKey = '';
            
            if (fileName.endsWith(`${baseOutputName}.umd.js`) && appState.rules.js) {
              matchedFileKey = `asset_${appState.rules.js}`;
            } else if (fileName.endsWith(`${baseOutputName}.css`) && appState.rules.css) {
              ...
              chrome.storage.local.get([matchedFileKey], (result) => {
                ...
                if (result[matchedFileKey]) {
                  if (typeof result[matchedFileKey] === 'string') {
                    ...
                    chrome.storage.local.get([`${matchedFileKey}_chunk0`], (chunkResult) => {
                      try {
                        const compressedData = chunkResult[`${matchedFileKey}_chunk0`];
                        const decompressedData = decompressData(compressedData);
                        
                        const blob = new Blob([decompressedData], { 
                          type: fileName.endsWith('.js') ? 'application/javascript' : 'text/css'
                        });
                        const reader = new FileReader();
                        reader.onload = () => {
                          sendResponse({
            ...
            chrome.storage.local.get([fileKey], (result) => {
              if (result[fileKey]) {
                ...
                if (typeof result[fileKey] === 'string') {
                  ...
                  // 同上，处理压缩数据
                  chrome.storage.local.get([`${fileKey}_chunk0`], (chunkResult) => {
                    try {
                      const compressedData = chunkResult[`${fileKey}_chunk0`];
                      const decompressedData = decompressData(compressedData);
                      
                      const blob = new Blob([decompressedData], {type: 'application/javascript'});
                      const reader = new FileReader();
                      reader.onload = () => {
                        sendResponse({
                          exists: true,
